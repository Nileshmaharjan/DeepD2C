...
def random_blur_kernel(probs, N_blur, sigrange_gauss, sigrange_line, wmin_line):
    N = N_blur
    coords = tf.compat.v1.to_float(tf.stack(tf.meshgrid(tf.range(N_blur), tf.range(N_blur), indexing='ij'), -1)) - (.5 * (N-1))
    # coords = tf.to_float(coords)
    manhat = tf.reduce_sum(input_tensor=tf.abs(coords), axis=-1)

    # nothing, default

    vals_nothing = tf.compat.v1.to_float(manhat < .5)
    ...
    vals_line = tf.exp(-dists**2/2./sig_line**2) * tf.compat.v1.to_float(manhat < w_line)
